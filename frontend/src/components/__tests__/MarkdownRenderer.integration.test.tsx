...
vi.mock('../../utils/diagramDetection', async () => {
  const actual = await vi.importActual('../../utils/diagramDetection');
  return {
    ...actual,
...
describe('MarkdownRenderer Integration Tests', () => {
  beforeEach(() => {
    ...
    mockMermaidRender.mockResolvedValue({
      svg: '<svg><text>Test Mermaid Diagram</text></svg>'
    });

    mockVegaLite.mockImplementation(({ spec }) => (
      <div data-testid="vega-lite-chart">
        Vega-Lite Chart: {spec?.mark || 'unknown'}
      </div>
    ));
  });
...
\`\`\``;
  ...
  describe('Diagram Detection and Loading', () => {
    it('detects mermaid diagrams and shows loading state', async () => {
...
\`\`\``;
    ...
    it('detects vega-lite charts and shows loading state', async () => {
...
\`\`\``;
    ...
    it('passes diagram theme configuration', () => {
      const content = `\`\`\`mermaid
graph TD
    A --> B
\`\`\``;
    ...
    it('passes zoom and pan configuration', () => {
      const content = `\`\`\`mermaid
graph TD
    A --> B
\`\`\``;
    ...
    it('passes vega actions configuration', () => {
      const content = `\`\`\`vega-lite
{"mark": "bar"}
\`\`\``;
      ...
      mockUseDiagramLoader.mockReturnValue({
        hasDiagrams: true,
        diagramInfo: { hasDiagrams: true, mermaidCount: 0, vegaLiteCount: 1, totalCount: 1 },
        libraryState: {
          mermaid: { loaded: false, loading: false, error: null },
          vegaLite: { loaded: true, loading: false, error: null },
        },
  ...
  describe('Error Handling', () => {
    it('handles markdown parsing errors gracefully with SafeMarkdownRenderer', () => {
      ...
      const content = 'Test content that should fallback';
      
      renderSafeWithProvider(content);
      
      // Should render fallback content
      expect(screen.getByText('Test content that should fallback')).toBeInTheDocument();
    ...
    it('logs errors with proper context information', () => {
      ...
      // Create a component that will throw during rendering
...
\`\`\``;
      ...
      expect(screen.getByText('console.log(\'This should work\');')).toBeInTheDocument();
...
`).join('\n')}`;
      ...
      mockDetectDiagrams.mockReturnValue({
        ...
        vegaLiteCount: 10,
        totalCount: 20,
        diagrams: Array.from({ length: 20 }, (_, i) => ({
      ...
      mockUseDiagramLoader.mockReturnValue({
        hasDiagrams: true,
        diagramInfo: { hasDiagrams: true, mermaidCount: 10, vegaLiteCount: 10, totalCount: 20 },
        libraryState: {
          mermaid: { loaded: false, loading: true, error: null },
          vegaLite: { loaded: false, loading: true, error: null },
        },
